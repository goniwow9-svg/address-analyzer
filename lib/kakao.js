...
export async function geocodeAddress(address) {
  ...
  // PNU(19자리 필지고유번호) = 시군구(5) + 법정동(5) + 산여부(1, 1=대지/2=산) + 본번(4) + 부번(4)
  // 토지이용계획정보(VWorld) 조회에 필요
  const mountainCode = addr.mountain_yn === "Y" ? "2" : "1";
  const pnu = `${sigunguCd}${bjdongCd}${mountainCode}${bun}${ji}`;
