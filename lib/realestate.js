...
export async function getRecentTransactions({ sigunguCd, bun, ji, buildingName, months = 6 }) {
  ...
  const matched = filterMatching(allRows, { bun, ji, buildingName });
...
export async function getYearlyPriceHistory({ sigunguCd, bun, ji, buildingName, years = 5 }) {
  ...
  return { years, totalMatched: matched.length, matchMethod: matched[0]?.matchMethod || null, yearly };
}
...
function filterMatching(rows, { bun, ji, buildingName }) {
  ...
  if (jibunMatched.length > 0) {
    return jibunMatched.map((r) => ({ ...r, matchMethod: "지번" }));
  }

  if (!buildingNorm) return [];
  ...
  return nameMatched.map((r) => ({ ...r, matchMethod: "건물명" }));
}
