...
export async function getYearlyPriceHistory({ sigunguCd, bun, ji, years = 5 }) {
  ...
  const matched = allRows.filter((row) => {
    const { main, sub } = parseJibun(row.jibun);
    return main === targetMain && sub === targetSub && row.cdealType !== "O";
  });
  ...
  return { years, totalMatched: matched.length, yearly };
}
