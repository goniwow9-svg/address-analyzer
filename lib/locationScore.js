...
async function scoreTraffic(x, y) {
  ...
  if (nearestSubway) {
    ...
    score += pts;
    details.push(`지하철 ${nearestSubway.name}까지 ${nearestSubway.distance}m (+${pts})`);
  ...
  // 버스정류장 근사치 (카카오에 전용 카테고리가 없어 키워드 검색으로 근사)
  const bus = await kakaoSearch({ type: "keyword", query: "버스정류장", x, y, radius: 500 });
  const busCount = bus.count;
  let busPts = 0;
  if (busCount >= 10) busPts = 8;
  else if (busCount >= 5) busPts = 5;
  else if (busCount >= 1) busPts = 2;
  score += busPts;
  details.push(`반경 500m 버스정류장 약 ${busCount}개 (+${busPts}, 카카오 키워드검색 근사치)`);
...
async function scoreEducation(x, y, region2) {
  ...
  if (nearestElem) {
    ...
    score += pts;
    details.push(`초등학교 ${nearestElem.name}까지 ${nearestElem.distance}m (+${pts})`);
  } else {
  ...
  if (nearestSecondary && nearestSecondary.distance <= 100 && !isGangnam3) {
    score -= 3;
    details.push(`${nearestSecondary.name}이 ${nearestSecondary.distance}m로 매우 근접 (-3)`);
  } else if (nearestSecondary && nearestSecondary.distance <= 100 && isGangnam3) {
    details.push(
      `${nearestSecondary.name}이 ${nearestSecondary.distance}m로 근접하지만 강남3구 학군 특성상 감점 미적용`
    );
  }
...
async function scoreNature(x, y) {
  ...
  if (nearestPark) {
    ...
    score += pts;
    details.push(`공원(${nearestPark.name})까지 ${nearestPark.distance}m (+${pts})`);
  } else {
  ...
  if (waterHit && waterHit.distance >= 500 && waterHit.distance <= 1000) {
    score += 3;
    details.push(`한강 접근성(${waterHit.name}) ${waterHit.distance}m (+3)`);
  }
...
async function scoreDeduction(x, y) {
  ...
  if (nearestReligious) {
    ...
    if (pts !== 0) {
      deduction += pts;
      details.push(`종교시설(${nearestReligious.name}) ${nearestReligious.distance}m (${pts})`);
    }
  }
