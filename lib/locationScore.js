...
import { getSeoulNearbyBusStops } from "./seoulBus";
...
// ── 교통 (25점) ──────────────────────────────────────────────
async function scoreTraffic(x, y, region1) {
  const details = [];
  let score = 0;
  ...
  // 버스정류장: 서울은 TAGO 커버리지가 부실해서 서울시 자체 API를 쓰고, 그 외 지역은 TAGO 사용
  const isSeoul = region1 === "서울" || region1 === "서울특별시";
  try {
    const busStops = isSeoul
      ? await getSeoulNearbyBusStops({ lat: y, lng: x, radius: 500 })
      : await getNearbyBusStops({ lat: y, lng: x });
    ...
    if (busStops.length > 0) {
      ...
      details.push(
        `반경 500m 버스정류소 ${within500.length}곳 — 가까운 순: ${names} (+${busPts}, ${
          isSeoul ? "서울시 API" : "TAGO"
        })`
      );
    } else {
      details.push(`주변 버스정류소를 찾지 못함 (+0, ${isSeoul ? "서울시 API" : "TAGO"})`);
    }
  } catch (err) {
...
export async function calculateLocationScore({ x, y, region1, region2 }) {
  const [traffic, convenience, education, nature, deduction] = await Promise.all([
    scoreTraffic(x, y, region1),
