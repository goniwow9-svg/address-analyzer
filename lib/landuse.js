...
export async function getLandUseInfo({ pnu }) {
  ...
  const url = `http://api.vworld.kr/ned/data/getLandUseAttr?${params.toString()}`;
  ...
  try {
    ...
    throw new Error(`토지이용계획 API 연결 실패: ${err.message}${cause}`);
  }
  const text = await res.text();

  if (!res.ok) {
    throw new Error(`토지이용계획 API 오류 (HTTP ${res.status}, pnu=${pnu}): ${text.slice(0, 200)}`);
  }
